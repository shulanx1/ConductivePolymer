function [decay_R, Rin_pooled1,Rin_pooled2] = sub_and_supra_ch1_dend(base_path, date, cell, file_name, idx_f, if_plot, if_save, save_path)
...
for i = 1:size(spike_waveform1,2)
    ...
    [~,locs,width,~]  = findpeaks(spike_waveform1(:,i), 'MinPeakProminence',1, 'MinPeakDistance',min([size(spike_waveform1,1)-2,floor(0.005/dt)]));
    if isempty(locs)
...
if if_plot
figure,plot(out.T, out.S{3}(:,1)-Vrest2(1))
hold on, plot(out.T, out.S{1}(:,1)-Vrest1(1))
xlim([0.25,1.25])
end
if if_save
