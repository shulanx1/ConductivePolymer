function [params,steady_ratio,AP_ratio, f, I,Rin_pooled1,Rin_pooled2] = sub_and_supra_dend_ch1(base_path, date, cell, file_name, idx_f, if_plot, if_save, save_path)
...
if abs(P1(1))>0.05
    ...
    dV2_t = filtfilt(b,a,diff(Vm1(floor(0.5/dt)+3:floor(0.5/dt)+500, 1)));
    ...
    V_sub1 = V_sub1-Vm1(floor(0.5/dt)+d_bridge,:)+Vm1(floor(0.5/dt),:);
    V_supra1 = V_supra1-Vm1(floor(0.5/dt)+5,:)+Vm1(floor(0.5/dt),:);
    Vm1_temp = Vm1;
    for i = 1:size(Vm2,2)
        Vm1_temp(floor(0.5/dt):floor(1/dt),i) = Vm1_temp(floor(0.5/dt):floor(1/dt),i)-Vm1_temp(floor(0.5/dt)+d_bridge,i)+Vm1_temp(floor(0.5/dt),i);
        Vm1_temp(floor(2/dt):floor(2.5/dt),i) = Vm1_temp(floor(2/dt):floor(2.5/dt),i)-Vm1_temp(floor(2/dt)+d_bridge,i)+Vm1_temp(floor(2/dt),i);
    end
    
    if if_plot
        figure,plot(out.T, Vm1_temp(:,1)-Vrest1(1))
        hold on
        plot(out.T, Vm1(:,1)-Vrest1(1))
    end
else
    Vm1_temp = Vm1;
end
if abs(P2(1))>0.05
V_sub2 = V_sub2-Vm2(floor(0.5/dt)+5,:)+Vm2(floor(0.5/dt),:);
V_supra2 = V_supra2-Vm2(floor(0.5/dt)+5,:)+Vm2(floor(0.5/dt),:);
end
...
Vsag = Vrest1(1)-Vm1_temp(floor(2/dt)+1:floor(2.5/dt), 1);
params(2) = max(Vsag(1: floor(0.2/dt)))-mean(Vsag(floor(0.4/dt):end)); % sag ratio
params(1) = Rin_pooled1;  % input resistance, MOhm
params(7) = mean(Ihold1);  % holding current, pA
params(8) = mean(Vrest1); % RMP, mV


decay_R = Rin_pooled2/Rin_pooled1;
...
i_fit_range = 11;
i_fit_range(i_fit_range>size(Vm1_temp,2)) = [];
...
for i = 1:length(i_fit_range)
    ...
    V_temp = Vm1_temp(floor(0.5/dt)+10:floor(0.65/dt), i_fit_range(i))-mean(Vm1_temp(1:floor(0.5/dt),i_fit_range(i)));
...
%         V_temp = Vm1_temp(floor(0.5/dt)+10:floor(0.75/dt), i_fit_range(i))-mean(Vm1_temp(1:floor(0.5/dt),i_fit_range(i)));
%         t_temp = [0:dt:dt*(length(V_temp)-1)]*1e3; %ms
%         opts = fitoptions( 'Method', 'NonlinearLeastSquares' );
%         opts.Display = 'Off';
